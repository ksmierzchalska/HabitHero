...
def get_top_users(limit=10):
    ...
    try:
        cursor.execute("""
            SELECT HANDLE, RANKING_POINTS, CURRENT_LEVEL
            FROM USERS
            WHERE RANKING_POINTS > 0  -- Tylko użytkownicy z punktami z wyzwań
            ORDER BY RANKING_POINTS DESC LIMIT %s
        """, (limit,))
...
def get_user_settings(user_id):
    ...
    try:
        ...
        print(f"Błąd przy pobieraniu ustawień w get_user_settings: {e}")
        disconnect_from_db(connection, cursor)
        return None
    ...
    try:
        ...
        cursor.execute("""
            ...
            LEFT JOIN HABIT_LOGS HL ON H.ID = HL.HABIT_ID
                AND HL.LOG_DATE >= DATE_ADD(NOW(), INTERVAL -%s DAY)
...
def get_weekly_progress(user_id):
    ...
    try:
        cursor.execute("""
            SELECT DATE_FORMAT(HL.LOG_DATE, '%%Y-%%m-%%d') AS date,
            ...
            WHERE H.USER_ID = %s AND HL.LOG_DATE >= DATE_ADD(NOW(), INTERVAL -7 DAY)
